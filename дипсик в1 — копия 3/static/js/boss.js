document.addEventListener('DOMContentLoaded', () => {
    ...
    function attachHandlers() {
      document.querySelectorAll('.brigade-option').forEach(opt => {
        opt.addEventListener('click', () => {
          document.querySelectorAll('.brigade-option').forEach(o => o.classList.remove('selected'));
      ...
      document.getElementById('startDate').addEventListener('change', updateChartAndStats);
      document.getElementById('endDate').addEventListener('change', updateChartAndStats);
    ...
    async function updateChartAndStats() {
      ...
      document.getElementById('periodRange').textContent = 
        `${formatDisplayDate(start)} - ${formatDisplayDate(end)}`; // Отображаем выбранные даты в сводной статистике
      document.getElementById('totalHouses').textContent = data.length;
      document.getElementById('totalApartments').textContent =
        data.reduce((s, r) => s + r.total_apartments, 0);
      document.getElementById('totalDone').textContent =
        data.reduce((s, r) => s + r.total_done, 0);
      ...
      chart = new Chart(document.getElementById('apartmentsChart').getContext('2d'), {
        type: 'bar',
      ...
      data.forEach(r => {
        ...
        tr.innerHTML = `
          ...
          <td>${foremanAddresses[r.foreman_id].size}</td>  <!-- Количество уникальных адресов (домов) -->
    ...
    function getColor(index, isBackground = false) {
      ...
      return isBackground ? colors[index % colors.length] + '80' : colors[index % colors.length];
    }
});
